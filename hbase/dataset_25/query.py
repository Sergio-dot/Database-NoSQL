...
def query1():
    ...
    table = connection.table("TRANSACTION_Q1_25")
...
def query2():
    start = time.time()
    connection = happybase.Connection()
    table = connection.table("TRANSACTION_Q2_25")
...
def query3():
    start = time.time()
    connection = happybase.Connection()
    table = connection.table("TRANSACTION_Q3_25")
    filter_a = "SingleColumnValueFilter('Transaction_Data', 'AMOUNT', >=, 'binary:1500')"
    filter_b = "SingleColumnValueFilter('Transaction_Data', 'DATE', >, 'binary:2015-12-31')"
...
# Replace results/time_qx_25.csv where x matches the number of the query
# example when assigning temp = query5() replace with results/time_q5_25.csv
# this will execute query5() and save the results under the results folder, in the time_q5_25.csv file

with open('results/time_q5_25.csv', mode='w', newline='') as csv_file:
