...
def query1():
    ...
    table = connection.table("TRANSACTION_Q1_50")
...
def query2():
    start = time.time()
    connection = happybase.Connection()
    table = connection.table("TRANSACTION_Q2_50")
...
def query3():
    start = time.time()
    connection = happybase.Connection()
    table = connection.table("TRANSACTION_Q3_50")
    filter_a = "SingleColumnValueFilter('Transaction_Data', 'AMOUNT', >=, 'binary:1500')"
    filter_b = "SingleColumnValueFilter('Transaction_Data', 'DATE', >, 'binary:2015-12-31')"
...
def query5():
    start = time.time()
    connection = happybase.Connection()
    table = connection.table("TRANSACTION_Q3_75")
    table_dict = {}
    filter_a = "SingleColumnValueFilter('Transaction_Data', 'AMOUNT', >=, 'binary:1500')"
    filter_b = "SingleColumnValueFilter('Transaction_Data', 'DATE', >, 'binary:2015-12-31')"
    filter_c = "SingleColumnValueFilter('Transaction_Data', 'STATUS', =, 'binary:Disputed')"
...
# Replace results/time_qx_50.csv where x matches the number of the query
# example when assigning temp = query5() replace with results/time_q5_50.csv
# this will execute query5() and save the results under the results folder, in the time_q5_50.csv file

with open('results/time_q5_50.csv', mode='w', newline='') as csv_file:
